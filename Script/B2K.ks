...
FUNCTION LogFlightSelected {
    ...
    SET thrustLog TO SHIP:THRUST.

    // --- OBLICZANE PARAMETRY ATMOSFERYCZNE ---
    SET machLog TO 0.
    SET tempLog TO 0.
    SET densLog TO 0.
    SET pressLog TO 0.
    ...
    IF BODY:ATM:EXISTS {
        // dane atmosfery
        SET T TO BODY:ATM:ALTITUDETEMPERATURE(altLog).
        SET gamma TO BODY:ATM:ADIABATICINDEX.
        SET Mmol TO BODY:ATM:MOLARMASS.
        SET P_atm TO BODY:ATM:ALTITUDEPRESSURE(altLog).

        // stała gazowa
        SET R_u TO 8.3144621.

        // konwersja ciśnienia na Pa
        SET P TO P_atm * 101325.

        // prędkość dźwięku
        SET c TO SQRT(gamma * R_u * T / Mmol).

        // Mach = prędkość powietrzna / prędkość dźwięku
        SET machLog TO SHIP:AIRSPEED / c.

        // gęstość z równania gazu doskonałego
        SET rho TO (P * Mmol) / (R_u * T).
...
LOG "TIME;ALT;LAT;LON;MASS;DYNPRESS;BEARING;HEADING;VERTSPEED;GROUNDSPEED;" +
    "ANGMOMX;ANGMOMY;ANGMOMZ;ANGVELX;ANGVELY;ANGVELZ;MACH;TEMP_K;AIRDENS;PRESS_PA;THRUST"
    TO log_file.
